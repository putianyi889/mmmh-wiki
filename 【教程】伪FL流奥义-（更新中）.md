...
如果操作保守一点，仅双击a-b，概率约80%的理想状态下手机收益比为`打开格子数/操作数=5/4=1.25`，电脑收益比为`打开格子数/操作数=5/2.5=2`；而概率约20%的c为雷状态下手机收益比为`打开格子数/操作数=2/4=0.5`，电脑收益比为`打开格子数/操作数=2/2.5=0.8`。加权平均的手机收益比为`1.25×79.4%+0.5×20.6%=1.1`，电脑收益比为`2×79.4%+0.8×20.6%=1.75`。因此在封闭局部下，无论手机或者电脑，试探双击两次都是有利的。可以考虑的后续操作是单击d下面1格，同时判断c是否可以双击，再补双击c或者改用nf。

手机上在a-b-c连续双击过程里安全的概率约是79.4%（可能踩雷的位置只有c，而电脑上无此顾虑）。在概率约63%的理想状态下（c-d都不是雷），手机收益比为`打开格子数/操作数=8/5=1.6`，电脑收益比为`打开格子数/操作数=8/3.5=2.29`；在概率约16.4%的差一点的情况下（c不是雷，d是雷），手机收益比为`打开格子数/操作数=5/5=1`，电脑收益比为`打开格子数/操作数=5/3.5=1.43`，也不亏；在概率为20.6%的最差情况下（c是雷），手机gg，电脑收益比为`打开格子数/操作数=2/3.5=0.57`。加权平均的手机收益比（生存情况下）为`(1.6×63%+1×16.4%+0×20.6%)/79.4%=1.17/79.4%=1.47`，电脑收益比为`2.29×63%+1.43×16.4%+0.57×20.6%=1.79`。

由于以上分析尚未加入在试探双击过程中判断出其他位置信息的潜在收益，因此实际收益更高。
...
仅双击a-b，概率约80%的理想状态下，手机收益比为`打开格子数/操作数=4/4=1`，电脑收益比为`打开格子数/操作数=4/2.5=1.6`；而概率约20.6%的c下方为雷的状态下，手机收益比为`打开格子数/操作数=2/4=0.5`，电脑收益比为`打开格子数/操作数=2/2.5=0.8`。加权平均的手机收益比为`1×79.4%+0.5×20.6%=0.9`，电脑收益比为`1.6×79.4%+0.8×20.6%=1.44`。

连双击a-b-c，在概率约63%的理想状态下，手机收益比为`打开格子数/操作数=7/5=1.4`，电脑收益比为`打开格子数/操作数=7/3.5=2`；在概率约16.4%的差一点的情况下，手机收益比为`打开格子数/操作数=4/5=0.8`，电脑收益比为`打开格子数/操作数=4/3.5=1.14`；在概率为20.6%的最差情况下，手机收益比为`打开格子数/操作数=2/5=0.4`，电脑收益比为`打开格子数/操作数=2/3.5=0.57`。加权平均的手机收益比为`1.4×63%+0.8×16.4%+0.4×20.6%=1.1`，电脑收益比为`2×63%+1.14×16.4%+0.57×20.6%=1.56`。
...
仅双击a-b，概率约63%的理想状态下，手机收益比为`打开格子数/操作数=5/4=1.25`，电脑收益比为`打开格子数/操作数=5/2.5=2`；而概率约37%的双击失败状态下手机收益比为`打开格子数/操作数=2/4=0.5`，电脑收益比为`打开格子数/操作数=2/2.5=0.8`。加权平均的手机收益比为`1.25×63%+0.5×37%=0.97`，电脑收益比为`2×63%+0.8×37%=1.56`。

连双击a-b-c，在概率约39.7%的理想状态下，手机收益比为`打开格子数/操作数=8/5=1.6`，电脑收益比为`打开格子数/操作数=8/3.5=2.29`；在概率约63%×37%=23.3%的差一点的情况下，手机收益比为`打开格子数/操作数=5/5=1`，电脑收益比为`打开格子数/操作数=5/3.5=1.43`；在概率约16.4%的次差情况下，手机除了绿色三格还能点开c格，手机收益比为`打开格子数/操作数=4/5=0.8`，电脑打开绿色三格，电脑收益比为`打开格子数/操作数=3/3.5=0.86`；在概率为20.6%的最差情况下，手机在c格踩雷gg，电脑收益比为`打开格子数/操作数=3/3.5=0.86`。加权平均的手机收益比（生存情况下）为`(1.6×39.7%+1×23.3%+0.8×16.4%+0×20.6%)/80%=0.9994/80%=1.25`，电脑收益比为`2.29×39.7%+1.43×23.3%+0.86×16.4%+0.86×20.6%=1.56`。

若故意把2点出再标雷连双击a-b，概率约80%的理想状态下，手机收益比为`打开格子数/操作数=5/5=1`，电脑收益比为`打开格子数/操作数=5/3.5=1.43`；而概率约20.6%的c下方为雷的状态下，手机收益比为`打开格子数/操作数=3/5=0.6`，电脑收益比为`打开格子数/操作数=3/3.5=0.86`。加权平均的手机收益比为`1×79.4%+0.6×20.6%=0.92`，电脑收益比为`1.43×79.4%+0.86×20.6%=1.31`。**均不如直接标雷连双击a-b的收益**。

若故意把2点出再标雷连双击a-b-c，在概率约63%的理想状态下，手机收益比为`打开格子数/操作数=8/6=1.33`，电脑收益比为`打开格子数/操作数=8/4.5=1.78`；在概率约16.4%的差一点的情况下，手机收益比为`打开格子数/操作数=5/6=0.83`，电脑收益比为`打开格子数/操作数=5/4.5=1.11`；在概率为20.6%的最差情况下，手机收益比为`打开格子数/操作数=3/6=0.5`，电脑收益比为`打开格子数/操作数=3/4.5=0.66`。加权平均的手机收益比为`1.33×63%+0.83×16.4%+0.5×20.6%=1.08`，电脑收益比为`1.78×63%+1.11×16.4%+0.66×20.6%=1.44`。**均不如直接标雷连双击a-b的收益**。
...
仅双击a，收益比为`打开格子数/操作数=2/1=2`。另外还有63%的净收益可能，收益比也是2，因此总体收益比仍然是2。
