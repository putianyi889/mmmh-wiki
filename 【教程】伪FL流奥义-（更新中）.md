...
如果操作保守一点，仅双击a-b，概率约79.4%的理想状态下手机效率（打开格子数/操作数）为`5/4=1.25`，电脑效率为`5/2.5=2`；而概率约20.6%的c为雷状态下，c右侧一格可以通过双击和下面一格同时打开，按半次操作计算，c左侧一格按nf计算，手机效率为`4/5.5=0.73`，电脑效率为`4/4=1`。加权平均的手机效率为`1.25×79.4%+0.73×20.6%=1.14`，电脑效率为`2×79.4%+1×20.6%=1.79`。因此在封闭局部下，无论手机或者电脑，试探双击两次都是有利的。可以考虑的后续操作是单击d下面1格，同时判断c是否可以双击，再补双击c或者改用nf，这样整体概率计算如下：79.4%可能连双击成功，其中又有79.4%概率可以补双击c，20.6%概率nf两格，手机效率为`8/5×79.4%+7/6×20.6%=1.51`，电脑效率为`8/3.5×79.4%+7/4.5×20.6%=2.14`；20.6%连双击失败，右侧两绿格可以通过一个补双击完成，其余格nf完成，手机效率为`7/8×79.4%+6/7×20.6%=0.87`，电脑效率为`7/6.5×79.4%+6/5.5×20.6%=1.08`。加权平均的手机效率为`1.51×79.4%+0.87×20.6%=1.38`，电脑效率为`2.14×79.4%+1.08×20.6%=1.92`。

手机上在a-b-c连续双击过程里安全的概率约是79.4%（可能踩雷的位置只有c，而电脑上无此顾虑）。在概率约63%的理想状态下（c-d都不是雷），手机效率为`8/5=1.6`，电脑效率为`8/3.5=2.29`；在概率约16.4%的差一点的情况下（c不是雷，d是雷），手机效率为`7/7=1`，电脑效率为`7/5.5=1.27`，也不亏；在概率为20.6%的最差情况下（c是雷），手机gg，电脑效率为`6/6.5×20.6%+7/7.5×79.4%=0.93`。加权平均的手机效率（生存情况下）为`(1.6×63%+1×16.4%+0×20.6%)/79.4%=1.17/79.4%=1.47`，电脑效率为`2.29×63%+1.27×16.4%+0.93×20.6%=1.84`。
...
仅双击a-b，概率约80%的理想状态下，手机收益比为`4/4=1`，电脑收益比为`4/2.5=1.6`；而概率约20.6%的c下方为雷的状态下，手机收益比为`2/4=0.5`，电脑收益比为`2/2.5=0.8`。加权平均的手机收益比为`1×79.4%+0.5×20.6%=0.9`，电脑收益比为`1.6×79.4%+0.8×20.6%=1.44`。

连双击a-b-c，在概率约63%的理想状态下，手机收益比为`7/5=1.4`，电脑收益比为`7/3.5=2`；在概率约16.4%的差一点的情况下，手机收益比为`4/5=0.8`，电脑收益比为`4/3.5=1.14`；在概率为20.6%的最差情况下，手机收益比为`2/5=0.4`，电脑收益比为`2/3.5=0.57`。加权平均的手机收益比为`1.4×63%+0.8×16.4%+0.4×20.6%=1.1`，电脑收益比为`2×63%+1.14×16.4%+0.57×20.6%=1.56`。

小结：**在半封闭局部带一格信息且首次双击格与雷斜向相邻时，无论手机或者电脑，试探双击三次都是有利的**。在首次双击格与雷斜向相邻且只能双击出4格时，手机上试探双击两次是亏损的，但电脑上可以这么选择。
...
仅双击a-b，概率约63%的理想状态下，手机收益比为`5/4=1.25`，电脑收益比为`5/2.5=2`；而概率约37%的双击失败状态下手机收益比为`2/4=0.5`，电脑收益比为`2/2.5=0.8`。加权平均的手机收益比为`1.25×63%+0.5×37%=0.97`，电脑收益比为`2×63%+0.8×37%=1.56`。

连双击a-b-c，在概率约39.7%的理想状态下，手机收益比为`8/5=1.6`，电脑收益比为`8/3.5=2.29`；在概率约63%×37%=23.3%的差一点的情况下，手机收益比为`5/5=1`，电脑收益比为`5/3.5=1.43`；在概率约16.4%的次差情况下，手机除了绿色三格还能点开c格，手机收益比为`4/5=0.8`，电脑打开绿色三格，电脑收益比为`3/3.5=0.86`；在概率为20.6%的最差情况下，手机在c格踩雷gg，电脑收益比为`3/3.5=0.86`。加权平均的手机收益比（生存情况下）为`(1.6×39.7%+1×23.3%+0.8×16.4%+0×20.6%)/80%=0.9994/80%=1.25`，电脑收益比为`2.29×39.7%+1.43×23.3%+0.86×16.4%+0.86×20.6%=1.56`。

若故意把2点出再标雷连双击a-b，概率约80%的理想状态下，手机收益比为`5/5=1`，电脑收益比为`5/3.5=1.43`；而概率约20.6%的c下方为雷的状态下，手机收益比为`3/5=0.6`，电脑收益比为`3/3.5=0.86`。加权平均的手机收益比为`1×79.4%+0.6×20.6%=0.92`，电脑收益比为`1.43×79.4%+0.86×20.6%=1.31`。**均不如直接标雷连双击a-b的收益**。

若故意把2点出再标雷连双击a-b-c，在概率约63%的理想状态下，手机收益比为`8/6=1.33`，电脑收益比为`8/4.5=1.78`；在概率约16.4%的差一点的情况下，手机收益比为`5/6=0.83`，电脑收益比为`5/4.5=1.11`；在概率为20.6%的最差情况下，手机收益比为`3/6=0.5`，电脑收益比为`3/4.5=0.66`。加权平均的手机收益比为`1.33×63%+0.83×16.4%+0.5×20.6%=1.08`，电脑收益比为`1.78×63%+1.11×16.4%+0.66×20.6%=1.44`。**均不如直接标雷连双击a-b的收益**。
...
由于本局部是开放式的，c格一列的浅绿两格都有可能是雷。由于高级雷密度为20.625%，c为雷的概率约为20.6%，浅绿格均没有雷的概率就是63%。黄格由于是在其他过程中判出可以双击的，属于净收益，收益比是3，可以双击的概率是(1-20.625%)^3=50%。

仅双击a，收益比为`2/1=2`。另外还有63%的净收益可能，收益比也是2，因此总体收益比仍然是2。

仅双击a-b，概率约63%的理想状态下收益比为`7/3×50%+4/2×50%=2.17`；而概率约37%的其他状态下收益比为`3/3=1`。加权平均的收益比为`2.17×63%+1×37%=1.74`。

双击a-b-c，概率约31.5%的理想情况（双击三次均成功）下收益比为`7/3=2.33`；概率约31.5%的次优情况（双击两次成功）下收益比为`4/3=1.33`；概率约16.4%的次差情况（双击一次成功且最后一次未踩雷）下手机收益比为`3/3=1`，电脑收益比为`2/3=0.67`；概率约20.6%的最差情况（双击一次成功且最后一次双击在雷上）下手机gg，电脑收益比为`2/3=0.67`。加权平均的手机收益比（生存情况下）为`(2.33×31.5%+1.33×31.5%+1×16.4%+0×20.6%)/79.4%=1.32/79.4%=1.66`，电脑收益比为`2.33×31.5%+1.33×31.5%+0.67×16.4%+0.67×20.6%=1.40`。
