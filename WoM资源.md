...
- 游戏胜利时获得宝石，受到装备“宝石”加成。红宝石、蓝宝石、黄玉、紫水晶要求经典模式且难度低于1000。缟玛瑙要求经典模式且难度不低于1000。
  ...
  - 连胜时可以获得紫水晶。初中高级紫水晶数量如下表[2]。尚不清楚自定义紫水晶公式，只知道短连胜大致成指数，且倍数和胜率有关。同胜率的图，紫水晶和难度成正比。
  ...
  - 效率较高时可以获得黄玉。初中高级黄玉数量如下表[3]。尚不清楚自定义黄玉数量公式。
...
非竞技场环境下，每1000难度3个缟玛瑙（高难），3个海蓝宝石（无猜），3个祖母绿（难度为1的图），0~5个红/蓝宝石（低难）。由于难度虚标，高难收入可以与竞技场相当。但是又由于有功勋点的存在，竞技场依然是最重要的资源途径。
...
**【内容已过期】**2022年3月12日更新：引擎船体的第四项改成门票
...
每个装备的每项属性最低是0%（不显示），最高是20%（未升级时）。装备的质量是该装备的五项属性之和。经验、金币、宝石、任务奖励、竞技场门票、竞技场奖励是连续属性，可以取到0%到20%的每个值。每日任务和赛季任务是离散属性，只要不为0%则作用都是+1。活动加成和每月的活动有关：轮盘活动和找票活动为连续属性（活动积分+x%），抢任务活动和送任务活动为离散属性（任务+1）。装备代码的中间五个字母/数字就代表着该装备五项的加成[1]：
- 0~9 表示 0%~9%
- X 表示 10%
- A~I 表示 11%~19%
- P 表示 20%
...
几个重要的节点：
|装备质量|20%|39%|48%|70%|80%|100%|完美L20|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
|强化花费|30|96|140|250|300|400|900|
...
### 滚雪球的功勋
对于可以反哺功勋的属性，如果活跃度足够高的话是可以滚雪球的：
- 对于所有游戏时带在身上的装备，如果一个月可以裸打25万游戏内经验则经验越高越好，否则越低越好。
- 对于声呐的竞技场奖励，如果一个月可以裸打25万竞技场经验则竞技场奖励越高越好，否则越低越好。

### 临时装备切换
有些装备属性只是偶尔有用，比如任务奖励（只在任务刷新时有用，换下后任务奖励不减少）、任务数量（换上一次就可以，换下后数量不会减少）、竞技场奖励（只在竞技场最后一把结算时有用）。因此可以准备一些专攻这些属性的蓝装，强化非常便宜而且同样能提供足够的加成。

未经特殊说明，下面给出的属性建议都是极限值。这类装备比较罕见，所以可以先弄个差不多的先用着。

#### 每日任务和赛季任务
由于每日任务和赛季任务的机制，引擎和船体可以备两套。这两套搭配起来可以达到完美的效果，而造价和强化花费加起来比完美要便宜不少。
- 一套是任务套，5%任意+20%任务奖励+1任务。
- 一套是资源套，金币19%+宝石20%。

每日任务和赛季任务刷新之前，换上任务套。任务刷新时是按照当时装备属性分配的任务和奖励，选好了任务之后再换回资源套，任务数量和奖励不变。每日任务在每天UTC0:00刷新，赛季任务在每月第一天UTC0:00刷新。**刷新赛季任务之前一定要记得换上任务套！**

#### 高难
打高难可以用两套定制装备：
- 找票套：鱼雷、雷达、声呐（-D）+30%门票，+19%经验。引擎、船体（-D）+30%经验，+5%其他。
- 结算套：五个装备（-O）全部+19%金币，+20%宝石，+20%缟玛瑙。

摸奖阶段用找票套，最大化该阶段的经验和门票收入。如果运气好要赢了，则换上**强化的**结算套然后收尾。

#### 活动任务
送任务活动的发送方无需任何游戏内收入，只要任务数量够多就行，所以可以用总品质20%且活动项非零的鱼雷和雷达，强化后任务数量即可+4（每件+2）。抢任务的部分任务需要游戏内收入，但是也不必强求（毕竟进不了前五就都差不多），可以用同样的策略。
