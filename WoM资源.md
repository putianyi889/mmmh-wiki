...
未经特殊说明，下面给出的属性建议都是极限值，并且需要用钻石合成。这类装备比较罕见，所以可以先弄个差不多的（也不需要钻石）先用着。
...
鱼雷和雷达也可以备任务套和资源套：
- 任务套：20%，含第五项活动加成，其他属性无所谓。
- 资源套：活动宝石门票经验能高则高，最好是四个20%，不要金币。如果要减少功勋压力则视重点活动克扣门票（轮盘活动）或宝石（找票活动）。

资源套用于冲找票活动和轮盘活动，因为这两个活动直接吃的是个人的资源量。任务套用于送任务活动和抢任务活动，因为这两个活动只在乎任务数量，别的属性都没用。就算某些任务需要吃资源的加成，也可以接到任务后再换上资源套做任务。按照目前的机制，如果任务可以+2，即使抢任务比较吃亏，总体来说还是很轻松可以打到25000。
